...
def rccm(x,  nclusts, lambda1, lambda2, lambda3=0, delta=0.001, max_iters=100, z0s=None, ncores=1):
   ...
   # Function for making almost symmetric matrix symmetric
    ...
    while np.max(abs(Omega0 - Omega0_old)) > delta or np.max(abs(Omegas - Omegas_old)) > delta or counter < 1:
        ...
        # load spcov package
        spcov = robjects.packages.importr('spcov')
        
        # loop over G
        for g in range(G):
            S0 = s0/ sum(wgk[g, :])
            penMat = robjects.r['matrix'](lambda3 / (lambda2 * sum(wgk[g, :])), nrow=p, ncol=p)
            penMat.do_slot_assign('diag', robjects.r['rep'](0, p))
            if counter > 1:
                Omega0[g,:, :] = spcov.spcov(Sigma=Omega0[g,:, :], S=S0, 
                                                 lambd=penMat, tol_outer=delta, step_size=100)[0]
            else:
                Omega0[g,:, :] = spcov.spcov(Sigma=r.solve(S0), S=S0, 
                                                 lambd=penMat, tol_outer=delta, step_size=100)[0]
            inv0[g,:, :] = np.linalg.inv(Omega0[g, :, :])
